import pytest

from matchescu.clustering._corr import WeightedCorrelationClustering
from pyresolvemetrics import twi
from tests.testutil import is_partition_over


@pytest.fixture
def corr(all_refs):
    return WeightedCorrelationClustering(all_refs)


def test_on_chain(corr, all_refs, chain_digraph):
    clusters = corr(chain_digraph)

    assert is_partition_over(all_refs, clusters)
    assert len(clusters) >= 2, "expected chain splitting"


def test_on_ring(corr, all_refs, ring_digraph):
    clusters = corr(ring_digraph)

    assert is_partition_over(all_refs, clusters)
    assert len(clusters) >= 2, "expected ring splitting"


def test_clique(corr, all_refs, clique_digraph):
    clusters = corr(clique_digraph)

    assert is_partition_over(all_refs, clusters)
    assert len(clusters) == 1, f"Clique should be 1 cluster, got {len(clusters)}"


@pytest.mark.parametrize(
    "all_refs",
    [
        [
            "a",
            "b",
            "c",
            "d",
            "e",
            "f",
            "g",
            "h",
            "i",
            "j",
        ]
    ],
    indirect=True,
)
def test_ring_with_cliques(corr, all_refs, ring_with_cliques_digraph):
    clusters = corr(ring_with_cliques_digraph)

    assert is_partition_over(all_refs, clusters)
    assert len(clusters) >= 3


@pytest.mark.skip(reason="only run this locally - not in CI")
def test_partitioning_on_real_data(
    matcher_mock, dataset_refs, dataset_ground_truth, dataset_fwd_graph
):
    algorithm = WeightedCorrelationClustering(dataset_refs, threshold=0.4)

    actual = algorithm(dataset_fwd_graph)

    assert is_partition_over(dataset_refs, actual)
    score = twi(dataset_ground_truth, actual)
    assert 0 <= score <= 1

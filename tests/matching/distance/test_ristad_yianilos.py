...
class StringEditDistanceLearner:
    def __init__(self):
    ...
    def compute_distance(self, x: str, y: str) -> float:
        alpha = self.forward_eval(x, y)
        return -np.log(alpha[len(x), len(y)])
...
def test_basic(abt, buy):
    corpus = list(abt.select("name").join(buy.select("name"), how="cross").head(1000).iter_rows())

    sedl = StringEditDistanceLearner()
    sedl.fit(corpus, 5)

    res = sedl.compute_distance("agata", "astana")
